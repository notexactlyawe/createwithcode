			...
			$('#accel_check :checkbox').change(function() {
				if (this.checked) {
					...
					$('#microbit').mousemove(function (event) {
						...
						const x_frac = ((event.pageX - offset.left) / width);
						const y_frac = ((event.pageY - offset.top) / height);
						// Micro:bit accel values are +/- 2000, let's do half for sensitivity
						const x = x_frac * 2000 - 1000;
						const y = y_frac * 2000 - 1000;
						mod.accelerometer.$d.data.x = x;
						mod.accelerometer.$d.data.y = y;

						// Between +/- 40 degrees rotation
						// Flip because the screen X (horiz) value should cause rotation about
						// Y axis
						const rotateX = y_frac * 80 - 40;
						const rotateY = x_frac * 80 - 40;
						$('#microbit').css("transform", `rotateX(${rotateX}deg) rotateY(${rotateY}deg)`);
					});
				} else {
					$('#microbit').off('mousemove');
					$('#microbit').css("transform", "");
				}
			});
