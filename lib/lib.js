...
var PythonIDE = {
	...
	debugHandler: function(result) {
		...
		if(PythonIDE.breakpoints[filename] && PythonIDE.breakpoints[filename][lineNumber] && PythonIDE.continueRunning) {
			PythonIDE.showHint("Paused due to breakpoint on line " + (lineNumber));
			$('#btn_showREPL').show();
			breakpoint = true;
...
//		console.log(PythonIDE.runMode, breakpoint);
					...
					for(var j = 0; j < tests.expect.length; j++) {
						...
						if(c.description == undefined) {
							switch(c.search) {
								...
								case "globals":
									if(c.name) {
										c.description = "Store " + c.for + " into a variable called " + c.name;	
									} else {
				...
				if(PythonIDE.lti && PythonIDE.lti.check && PythonIDE.lti.check) {
					if(PythonIDE.lti.submitted != score) {
						$.getJSON('/lib/api.php?cmd=lti_report_grade', {
							...
							if(e.success) {
								PythonIDE.showHint("Saved score: " + score + "%");
								PythonIDE.lti.submitted = score;
							} else {
								PythonIDE.showHint("Score not saved");
								console.log(e);
							}
						});	
					}
	...
	downloadFile: function() {
		...
		saveAs(blob, PythonIDE.currentFile);
	},
	...
	save: function() {
		...
		$.post('/lib/api.php', {
			...
			if(data.verified) {
				...
				PythonIDE.showHint("Could not save: Invalid captcha data");
			}
		}, "json");
	},
	...
	configSkulpt: function(mode, forceDebugging) {
		...
		Sk.inputfun = function(prompt) {
			...
			var p = new Promise(function(resolve, reject) {
				...
				PythonIDE.python.output('<form><div id="raw_input_holder"><label for="raw_input">' + prompt + '</label><input class="ui-widget ui-state-default ui-corner-all" type="text" name="raw_input" id="raw_input" value=""/><button id="raw_input_accept" type="submit">OK</button></div></form>');
			...
			if(id == PythonIDE.groups[i].id) {
				...
				$('#btnGroup').hover(function(e) {
					...
					if(PythonIDE.currentGroup) {
						...
						PythonIDE.api('get_group_code', {group:g.id}, function(r) {
							html = '';
							if(r.success && r.activities && r.activities.length > 0) {
								var html = '<table class="testing_table"><thead><tr><th>Date</th><th>Name</th><th>Instructions</th></tr></thead><tbody>';
								for(var i = 0; i < r.activities.length; i++) {
									var a = r.activities[i];
									html += '<tr><td>' + a.date + '</td><td><a href="' + a.url + '" target="_blank">' + a.name + '</a></td><td><pre>' + a.instructions + '</pre></td></tr>';
								}
							}
							html += '</tbody></table>';
							$('#code_shared').html(html);
							html = '';
						...
						$('#dlg').dialog('option', 'title', g.name);
						var html = '<a target="_blank" href="' + g.link + '">Show all group resources</a><fieldset><legend>Code shared with me:</legend><div id="code_shared">Loading...</div></fieldset>';
						html += '<fieldset><legend>My recent code:</legend><div id="code_mine">Loading...</div></fieldset>';
						$('#output').html(html);
						$('#dlg').dialog("open");
					}		
				});
	...
	init: function(style) {
		PythonIDE.api('get_user_details', {}, function(r) {
			if(r.success && r.user && r.user.username) {
				var html = 'You are logged in as <a href="https://tools.withcode.uk/keywords/user" target="_blank">' + r.user.username + "</a>";
				if(!r.user.student) {
					html += ' (teacher)';
				}
				if(r.groups) {
					...
					for(var i = 0; i < r.groups.length; i++) {
						var g = r.groups[i];
						html += '<option value="' + g.id + '">' + g.name + ' (' + g.schoolname + ')</option>';
					}
					html += '</select>';
				}
				$('#kwuser').html(html);
				...
				$('#kwuser_group').selectmenu({
					change: function(e, ui) {
						PythonIDE.changeGroup(parseInt(ui.item.value));
					}
				});
			}
		});
		...
		function loadFile(file) {
			...
			if (file.name.match(/\.(txt|py|json)$/)) {
				...
				if(file.name.match(/\.sb3$/)) {
					PythonIDE.showHint("Attempting to read sb3 file");
				} else {
					PythonIDE.showHint("Unsupported file format");	
				}
		...
		$('.btn_tracker_tool').button().click(function(e) {
			switch(e.currentTarget.id) {
				case 'btn_copy_tracker':
					...
					PythonIDE.showHint('Tracker code copied to clipboard');
				break;
						...
						switch(e.currentTarget.id) {
							case 'dlg_confirm_ok':
								$.getJSON('/lib/api.php', {
									...
									PythonIDE.showHint("Tracker deleted");
									PythonIDE.updateTrackerList();
								});
							break;
						}
		...
		function addSavedCodePreviewHandlers() {
			...
			$('.codepreview').off('mouseenter dblclick mouseleave').on({
				'mouseenter': function(e) {
					...
					PythonIDE.showHint("Loading preview..." + hash);
					$.getJSON('/lib/api.php', {
						cmd: 'get_code',
						hash: hash
					}, function(data) {
						if(data.success) {
							PythonIDE.showHint("Previewing code: " + data.hash);
							var name = 'mycode.py';
							if(data.name) {
								name = data.name;
							}
							...
							PythonIDE.updateFileTabs();
							PythonIDE.showHint("Could not load code preview");
						}
					});
				},
				'dblclick': function(e) {
			...
			switch(e.currentTarget.id) {
				case 'btn_project_ok':
					...
					if(!newProjectName.match(/^[A-Za-z0-9_ ]+$/)){
						PythonIDE.showHint("Invalid project name");
						break;
					}
		...
		$('#btnSaveNew').button().click(function() {
			...
			if(captcha != "") {
				...
				PythonIDE.showHint("Please tick the 'I'm not a robot' box");
			}			
		});
		...
		Sk.externalLibraries = {
			...
			hashlib: {
				path: '/lib/skulpt/hashlib/__init__.js',
		...
		if(PythonIDE.hash && PythonIDE.aT && PythonIDE.aT[PythonIDE.hash] && PythonIDE.aT[PythonIDE.hash].c) {
			...
			if(JSON.stringify(PythonIDE.aT[PythonIDE.hash].c) != PythonIDE.defaultSavedCode) {
				...
				PythonIDE.editor.setValue(PythonIDE.files[PythonIDE.currentFile]);
				PythonIDE.updateFileTabs();
				PythonIDE.showHint("Displaying code you last edited " + timeSince(PythonIDE.aT[PythonIDE.hash].t) + " ago");	
				var html = '<img id="btn_reset_code" alt="Reset code to saved version" title="Reset code" class="toolButton hiddenButton visibleButton" src="/media/reset.png">';
	...
	resetFiles: function(all) {
		if(PythonIDE.aT && PythonIDE.hash && PythonIDE.aT[PythonIDE.hash] && PythonIDE.defaultSavedCode) {
			...
			PythonIDE.editor.setValue(PythonIDE.files[PythonIDE.currentFile]);
			PythonIDE.updateFileTabs();
			PythonIDE.showHint("Reset to default saved code. Use Code Recovery button to undo.");
		}
	}
}
